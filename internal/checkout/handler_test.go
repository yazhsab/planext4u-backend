...
func TestPhase3CustomerTransactionAPI(t *testing.T) {
	...
	event := payment.ProviderEvent{EventID: "handler-provider-event-001", PaymentID: placed.Payment.ID, ProviderReference: placed.Payment.ProviderReference, Status: payment.StatusCaptured, AmountMinor: quote.Total.AmountMinor, Currency: quote.Total.Currency}
	eventBody, _ := json.Marshal(event)
	webhookRequest := httptest.NewRequest(http.MethodPost, "/v1/payments/webhooks/razorpay", bytes.NewReader(eventBody))
	webhookRequest.Header.Set("X-Planext4u-Signature", payment.Sign(fixture.secret, eventBody))
