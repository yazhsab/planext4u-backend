...
module "delivery_identity" {
  ...
  task_role_arns           = concat([for service in keys(var.services) : "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/${var.name}-${var.environment}-${service}-task"], ["arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/${var.name}-${var.environment}-ecs-execution"])
